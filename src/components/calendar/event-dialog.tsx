  ...
  const initial =
    state.mode === "create"
      ? {
          ...
          day: startOfDay(state.time),
          timeMin: minutesIntoDay(state.time),
          range: { from: startOfDay(state.time), to: startOfDay(state.time) },
          calendarId: null,
          categoryId: null,
        }
      : {
          ...
          departmentId: state.event.departmentId ?? null,
          allDay: isAllDay(state.event),
          day: isAllDay(state.event)
            ? parseDayKey(state.event.startDate)
            : startOfDay(new Date(state.event.time!)),
          timeMin: isAllDay(state.event)
            ? 9 * 60
            : minutesIntoDay(new Date(state.event.time!)),
          range: isAllDay(state.event)
            ? {
                from: parseDayKey(state.event.startDate),
                to: parseDayKey(state.event.endDate),
              }
            : {
                from: startOfDay(new Date(state.event.time!)),
                to: startOfDay(new Date(state.event.time!)),
              },
          calendarId: state.event.calendarId ?? null,
          ...
          <div className="flex min-w-0 flex-1 flex-col gap-4">
            ...
            {allDay ? (
              ...
              <div className="grid grid-cols-[1fr_auto] items-end gap-2">
                ...
                <div className="flex flex-col gap-2">
                  <Label htmlFor="event-time" className="text-muted-foreground">
                    Time
                  </Label>
                  <TimePicker
                    id="event-time"
